...
async function displayAlbums() {
    ...
    for (const link of anchors) {
        ...
        const parts = url.pathname.split("/").filter(Boolean); // ["songs", "NCS"]

        // we only want paths like /songs/<folder>/
        if (parts.length === 2 && parts[0] === "songs") {
            const folder = parts[1]; // "NCS", "Arijit", etc.
            console.log("Folder:", folder);

            try {
                const infoRes = await fetch(`/songs/${folder}/info.json`);
                if (!infoRes.ok) {
                    console.error("No info.json for", folder);
                    continue;
                }
